...
.triggerHandler('showStep', [null, selectedStep, null, null]) // move progressbar to step 1 (0 index)
 ...
 $('#validation-form').validate({
   ...
   */
  var init = function () {
    var task = getTask(getQueryVariable("id"))
    var script = getScript(task.scriptName)
    initParam(script,task)
  }
   ...
   * @param script
   */
  var initParam = function(script,task){
    $("#parameter").empty()
    if(JSON.stringify(script.parameters) != "{}"){
      for (let parameter in script.parameters){
        ...
        $("#parameter").append("<div class=\"form-group form-row mt-4\">" +
            ...
            "<input id=\""+parameter+"\" type=\"text\" class=\"form-control form-control-lg brc-blue-m2 pl-45\" placeholder=\"\" value=' "+ task.parameters[parameter] + "' />" +
            "</div>" +
