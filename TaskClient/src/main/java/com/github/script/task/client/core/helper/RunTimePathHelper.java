...
import com.github.script.task.bridge.util.SystemUtil;
...
public class RunTimePathHelper {
     ...
     */
//    @Autowired
    private void closeRunTime(ApplicationContext applicationContext) {
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            //结束所有进程
            if (SystemUtil.isWindows()) {
                try {
                    File runTimeHomeFile = new File(new ApplicationHome().getDir().getAbsolutePath() + "/" + scriptTaskConf.getRunTime().getHome());
                    String runTimePath = FilenameUtils.normalize(runTimeHomeFile.getAbsolutePath()).replaceAll("\\\\", "\\\\\\\\");
